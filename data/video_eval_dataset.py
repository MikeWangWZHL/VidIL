...
class VideoDatasetEvalRetrieval(Dataset):
    ...
    def __init__(self, 
        ...
        '''
        print('initialize eval video retrieval dataset...')
        original_ann = load_jsonl(ann_jsonl)
        self.annotation = []
        skip_count = 0 
        for item in original_ann:
            video_path = os.path.join(video_root,item['clip_name'] + video_fmt)
            if os.path.exists(video_path):
                self.annotation.append(item)
            else:
                skip_count += 1
        print('original ann len:',len(original_ann))
        print('skip non-exist video num:',skip_count)
        ...
        # visual encoder version: deciding output video dimension order
        self.vit = vit         
...
class VideoDatasetEvalCaption(Dataset):
    ...
    def __init__(self, 
        ...
        # load ann
        print('initialize eval video captioning dataset...')
        lines = load_jsonl(ann_jsonl)
        video_2_gt = defaultdict(list) # list of reference captions
        for line in lines:
            video_2_gt[line['clip_name']].append(line['caption'])

        self.annotation = []
        skip_count = 0
        for key,value in video_2_gt.items():
            video_path = os.path.join(self.video_root, key + self.video_fmt) 
            if os.path.exists(video_path):
                self.annotation.append({'clip_name':key,'caption':value})
            else:
                skip_count += 1
        print('original test ann len:',len(video_2_gt))
        print('skip non-exist video num:',skip_count)
