...
INSTRUCTION_LINE="Generate a video caption based on the objects, events, attributes, frame captions and subtitle. Example:"
SHOT=$4
RANDOM_SEED=$5
N=$6
COMPARING_TARGET=$7 # use which part's similarity to select in-context example
...
then
    ...
    ASR_PATH="${SHARED_DATASETS}/Youcook2/value_ann/videoid_2_subtitle_yc2c.json"
fi

if [ $DATASET == "vatex" ]
then
    TRAINSET_JSON_ANN="${SHARED_DATASETS}/Vatex/value_ann/vatex_en_c/videoid_2_text_vatex_en_c_train.json" # vatex
    ASR_PATH="${SHARED_DATASETS}/Vatex/value_ann/videoid_2_subtitle_vatex.json"
fi
...
PROMPT_OUTPUT_DIR="$OUTPUT_DIR/input_prompts_caption_in_context_selection_asr"
...
# generate prompt
OUTPUT_NAME="gpt3_queries_${DATASET}_${SPLIT}_${PROMPT_TASK}_with_in_context_selection_ASR_${VISUAL_TOKENIZATION_ENCODER}_shot_${SHOT}_seed_${RANDOM_SEED}_N_${N}.jsonl"

python generate_prompts_random_prefix_in_context_selection.py \
...
--shot $SHOT \
--N $N \
--comparing_target $COMPARING_TARGET
