...
class BLIP_Retrieval(nn.Module):
    ...
    def forward(self, image, caption, alpha, idx):
        ...
        with torch.no_grad():
            ...
            # print('sim_i2t_m size:', sim_i2t_m.size())
            # print('sim_t2i_m size:', sim_t2i_m.size())
            # print('sim_targets size:', sim_targets.size())
...
class BLIP_Retrieval_Video(nn.Module):
    def __init__(self,                 
                 med_config = 'configs/med_config.json',  
                 image_size = 384,
                 vit = 'base',
    ...
    def forward(self, video, caption, alpha, idx):
        ...
        video_feat = video_feat.view(B,N,-1).mean(dim=1) # (B,embed_dim) ([B, 256])
        ...
        with torch.no_grad():
            ...
            video_feat_m_all = torch.cat([video_feat_m.t(),self.image_queue.clone().detach()],dim=1) # transpose and concat           
...
def blip_retrieval_video(pretrained='',**kwargs):
    model = BLIP_Retrieval_Video(**kwargs)
    if pretrained:
