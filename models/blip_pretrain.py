...
class BLIP_Pretrain_Video(nn.Module):
    def __init__(self,                 
                 med_config = 'configs/bert_config.json',  
                 image_size = 224,
                 vit = 'base',
    ...
    def forward(self, video, caption, alpha):
        with torch.no_grad():
            self.temp.clamp_(0.001,0.5)
        ...
        video_feat = self.vision_proj(video_embeds[:,0,:]) # CLS: (BxN,embed_dim)
        video_feat = video_feat.view(B,N,-1).mean(dim=1) # (B,embed_dim)
        video_feat = F.normalize(video_feat,dim=-1)

        video_embeds = video_embeds.view(B,-1,video_embeds.shape[-1]) # (B,NxP,vision_witdth)
        ...
        with torch.no_grad():
            ...
            video_feat_m = self.vision_proj_m(video_embeds_m[:,0,:]) # CLS: (BxN,embed_dim)
            video_feat_m = video_feat_m.view(B,N,-1).mean(dim=1) # (B,embed_dim)
            video_feat_m = F.normalize(video_feat_m,dim=-1)
            video_embeds_m = video_embeds_m.view(B,-1,video_embeds_m.shape[-1]) # (B,NxP,vision_witdth)
            video_feat_all = torch.cat([video_feat_m.t(),self.image_queue.clone().detach()],dim=1) # transpose and concat                   
...
def blip_pretrain_from_pretrained(pretrained=None,**kwargs):
    model = BLIP_Pretrain(**kwargs)
    if pretrained:
...
def blip_video_pretrain(pretrained=None,**kwargs):
    model = BLIP_Pretrain_Video(**kwargs)
    if pretrained:
        model,msg = load_checkpoint(model,pretrained)
        print("missing keys:")
        print(msg.missing_keys)
